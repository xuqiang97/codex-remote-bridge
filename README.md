...
> **V1 实现与验收进行中，尚未正式发布。** Windows 已验证恢复一个 Desktop 释放后的真实旧任务并延续上下文。Desktop 仍持有 writer 的任务会被拒绝，即使上一轮已经结束。不能承诺“所有任务随时接管”。[实际验证记录](docs/VALIDATION.md)区分本机结果、CI 与未完成的验收。
